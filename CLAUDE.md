...
教程里**所有公式、知识点、可视化都必须严谨**。为了好懂**可以简化、可以粗略(toy 维度、省略次要项、近似示意)**,
但**绝不能是错的**。简化要诚实标注(如"真实是几千维""这里略去 xx"),不能因简化而引入错误结论。
拿不准的事实/架构细节先核实(查权威资料)再写;宁可少讲,不可讲错。

> 已对全教程做过一轮权威核对(见 `docs/教程核对报告.md`,217 条逐条带引用)。
> **新增/改动内容比照执行**:关键论断要能落到 `src/data/refs.js` 的某条文献,并在章末 `<Refs>` 引用。
