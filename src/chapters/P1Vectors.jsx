...
export default function P1Vectors({ prev, next }) {
  ...
  const renderVec = (cell) => {
    ...
    const by = 40
    els.push(<text key="ct" x={bx + 16} y={by - 8} textAnchor="middle" fontFamily={T.font} fontSize={11} fill={T.c.dim}>= 一串数字</text>)
    ;[vx, vy].forEach((v, i) => {
      ...
      els.push(<text key={`crl${i}`} x={bx + 56} y={by + i * 30 + 18} fontFamily={T.font} fontSize={9} fill={T.c.dim}>维{i}</text>)
    })
    els.push(<text key="info" x={50} y={cy + R * unit + 22} fontFamily={T.font} fontSize={11} fill={T.c.accent2}>
      长度 |v| = √({vx}²+{vy}²) = {len.toFixed(2)} ,方向 = {ang.toFixed(0)}°</text>)
    const W = bx + 90
  ...
  const renderMat = (cell) => {
    ...
    els.push(<text key="cl0" x={left + cs / 2} y={top - 6} textAnchor="middle" fontFamily={T.font} fontSize={9} fill={T.c.dim}>维0</text>)
    els.push(<text key="cl1" x={left + cs + cs / 2} y={top - 6} textAnchor="middle" fontFamily={T.font} fontSize={9} fill={T.c.dim}>维1</text>)
    els.push(<text key="mc" x={left} y={top + MAT.length * cs + 16} fontFamily={T.font} fontSize={10} fill={T.c.dim}>
      矩阵 {MAT.length}×2:每行是一个 token 的向量</text>)
    // 右:把选中行画成箭头
    const R = 2
    const unit = cs * 1.4
    ...
    els.push(<text key="rc" x={cx - R * unit} y={cy + R * unit + 18} fontFamily={T.font} fontSize={10} fill={T.c.dim}>
      点左边的行 → 看它在平面上的箭头(每个 token 是空间里的一个点)</text>)
  ...
  const ctl1 = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 13 }}>
      <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ color: 'var(--text-dim)', width: 40 }}>维0 x</span>
      ...
      <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ color: 'var(--text-dim)', width: 40 }}>维1 y</span>
  ...
  const ctl2 = (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, flexWrap: 'wrap' }}>
      <span style={{ color: 'var(--text-dim)' }}>选 token 行</span>
      {TOK.map((t, i) => (
  ...
  return (
    <ChapterLayout kicker="预备知识 · P1" title="向量与矩阵是什么" prev={prev} next={next}>
      <>
        <p>
      ...
      <>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-dim)', margin: '4px 0 10px' }}>
          拖动两个分量,左边箭头和右边那串数字始终是同一个向量;下方是它的长度与方向。
        </p>
        <FigureBoard renderSvg={renderVec} baseCell={22} fullCell={34} controls={ctl1} />

        <h3 style={{ marginTop: 18 }}>图 2 · 矩阵 = 一摞 token 向量</h3>
        <p style={{ fontSize: 13, color: 'var(--text-dim)', margin: '4px 0 10px' }}>
          左:一句话的 n×d 矩阵(每行一个 token)。点某行 → 右边平面画出它的箭头。
        </p>
