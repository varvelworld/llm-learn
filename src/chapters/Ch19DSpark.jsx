    ...
    <ChapterLayout kicker="第二部分 · DeepSeek-V4 · Ch19" title="DSpark(一)· 投机解码与半自回归" prev={prev} next={next}>
      <>
        ...
        <div className="note">
          ...
          大模型想要 <Tex>{'p_t=[0.5,\\ 0.3,\\ 0.2]'}</Tex>、草稿给 <Tex>{'p_d=[0.2,\\ 0.6,\\ 0.2]'}</Tex>。
          <ul style={{ margin: '4px 0' }}>
            <li>逐字相减 <Tex>{'p_t-p_d=[+0.3,\\ -0.3,\\ 0]'}</Tex>;</li>
            <li><b>取正部</b>(负的当 0)→ <Tex>{'[0.3,\\ 0,\\ 0]'}</Tex>:只有 <b>A</b> 是「草稿给少了」;</li>
            <li>归一化(÷0.3)→ <Tex>{'[1,\\ 0,\\ 0]'}</Tex>:所以这次<b>补采就采 A</b>。</li>
          </ul>
          直觉:补采只在「<b>草稿亏欠</b>」的字里挑——给多了的(B)不补、持平的(C)不补。
        </div>
