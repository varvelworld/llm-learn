...
// 估算一段文本的像素宽(CJK≈11、ASCII≈6.4),用来给 SVG 留够宽度、防英文更长时裁切。
...
export default function Ch06FFN({ prev, next }) {
  ...
  const renderSwiGLU = (cell) => {
    ...
    const els = []
    let cx = 24
    let maxRight = 0 // 跟踪最右文本边界,英文标题更长时防裁切
    const blk = (row, yc, { title, vmax, dec = 1, stroke }) => {
      ...
      maxRight = Math.max(maxRight, cx + row.length * C, cx + (row.length * C) / 2 + estTextW(title) / 2)
    ...
    // 输入 x
    const xB = blk(x, cy, { title: t(`x「${tokens[f]}」(${D}维)`, `x「${tokens[f]}」(${D}-dim)`), vmax: vmaxD, stroke: T.c.accent })
    cx = xB.cx1 + 44
    // 两支:门(上) / 值(下)
    const gateX = cx
    const gB = blk(sw.gate, cy - rg, { title: t('门 = Swish(x·W_g)', 'gate = Swish(x·W_g)'), vmax: 1.2, dec: 2, stroke: T.c.warn })
    const uB = blk(sw.up, cy + rg, { title: t('值 = x·W_u', 'value = x·W_u'), vmax: vmaxF })
    // x 到两支的连线
    ...
    // 门 ⊙ 值 = 隐藏
    const hB = blk(sw.hidden, cy, { title: t('门 ⊙ 值 = 隐藏(升维)', 'gate ⊙ value = hidden (expand)'), vmax: vmaxF, stroke: T.c.accent2 })
    cx = hB.cx1 + 12
    ...
    cx += 46
    blk(sw.out, cy, { title: t('输出(降回 d)= Δ', 'output (back to d) = Δ'), vmax: vmaxD, stroke: T.c.accent2 })
    cx += D * C + 16
    const W = Math.max(cx, maxRight + 8)
    return <svg width={W} height={cy + rg + C + 16} style={{ display: 'block', minWidth: W }}>{els}</svg>
  }
  ...
  const renderActivation = () => {
    ...
    const leakText = t('负区有小幅泄漏(不像 ReLU 直接归零)', 'small leak in the negative region (unlike ReLU which zeros out)')
    const botText = t('Swish(z)=z·σ(z),平滑可导;门控用它当"软开关"', 'Swish(z)=z·σ(z), smooth & differentiable; used as a "soft switch" in gating')
    const svgW = Math.max(w + 90, px(-3.5) + estTextW(leakText) + 8, ox + estTextW(botText) + 8)
    return (
      <svg width={svgW} height={h + 50} style={{ display: 'block', minWidth: svgW }}>
        ...
        <text x={px(-3.5)} y={py(-1.1)} fontFamily={T.font} fontSize={9} fill={T.c.accent2}>{leakText}</text>
        <text x={ox} y={oy + h + 22} fontFamily={T.font} fontSize={9} fill={T.c.dim}>{botText}</text>
      </svg>
    )
  }
  ...
  const renderContrast = () => {
    ...
    const els = []
    const attnTitle = t('注意力:跨 token 混合', 'Attention: mixes across tokens')
    const ffnTitle = t('FFN:逐 token 独立', 'FFN: per-token, independent')
    const fnNote = t('同一个 FFN,各 token 各过各的,互不影响', 'Same FFN; each token goes through on its own, no interaction')
    // 左:注意力 跨 token
    let x0 = 20
    els.push(<text key="al" x={x0} y={14} fontFamily={T.font} fontSize={11} fill={T.c.accent}>{attnTitle}</text>)
    cols.forEach((t2, i) => els.push(...box(x0 + i * (C + gap), top, t2, T.c.dim)))
    cols.forEach((t2, i) => els.push(...box(x0 + i * (C + gap), top + 70, t2, T.c.accent)))
    ...
    els.push(<text key="fl" x={x1} y={14} fontFamily={T.font} fontSize={11} fill={T.c.accent2}>{ffnTitle}</text>)
    cols.forEach((t2, i) => els.push(...box(x1 + i * (C + gap), top, t2, T.c.dim)))
    cols.forEach((t2, i) => els.push(...box(x1 + i * (C + gap), top + 70, t2, T.c.accent2)))
    cols.forEach((t2, i) => els.push(<line key={`f${i}`} x1={x1 + i * (C + gap) + C / 2} y1={top + C} x2={x1 + i * (C + gap) + C / 2} y2={top + 70} stroke={T.c.accent2} strokeWidth={1.2} />))
    els.push(<text key="fn" x={x1} y={top + 70 + C + 16} fontFamily={T.font} fontSize={9} fill={T.c.dim}>{fnNote}</text>)
    const W = Math.max(x1 + 3 * (C + gap) + 20, x1 + estTextW(fnNote) + 8, x1 + estTextW(ffnTitle) + 8, x0 + estTextW(attnTitle) + 8)
    return <svg width={W} height={top + 70 + C + 26} style={{ display: 'block', minWidth: W }}>{els}</svg>
  }
  ...
  return (
    <ChapterLayout
      kicker={t('第 6 章 · 前馈 FFN / SwiGLU', 'Chapter 6 · FFN / SwiGLU')}
      title={t('前馈网络 FFN / SwiGLU', 'FFN / SwiGLU')}
      prev={prev}
      next={next}
      translated
    >
      <>
        {lang === 'en' ? (
          <>
            <p>
              A Transformer layer has two sublayers: <b>attention</b> (moves information across tokens) and the <b>FFN</b> (processes each token).
              The last chapter showed how they hang on the residual stream; this one looks at what happens <b>inside</b> the FFN.
            </p>
            <h2>What an FFN really is: per-token, expand then project back</h2>
            <p>
              The FFN <b>does not mix tokens</b> — each token's vector goes <b>independently</b> through the same small network (see the contrast figure).
              A standard FFN is <code>W₂·relu(W₁·x)</code>: it <b>expands</b> d up to a larger d_ff,
              applies an activation, then <b>projects back</b> to d. Toy here: {D} → {DFF} → {D}.
            </p>
            <div className="note">
              <b>What is the point of expanding?</b> d_ff (usually 4d) = how many <b>patterns this layer can detect / store at once</b>:
              each hidden unit is an independent <b>feature detector</b> — each row of <code>W₁</code> detects some pattern in the input,
              the activation decides <b>how strongly it fires</b>, and each column of <code>W₂</code> adds that unit's <b>stored content</b> back.
              Wider = more detectors = more it can recognize / remember. A classic view: the FFN is a <b>key-value memory</b>
              (rows of W₁ = keys, columns of W₂ = values), where <b>most facts and rules live</b>, which is why the FFN holds most of a model's parameters.
              Intuition: <b>expand = unfold and think it over, project back = fold the thought into one update</b>.
            </div>
            <h2>SwiGLU: an FFN with a "gate" (used by LLaMA / DeepSeek)</h2>
            <p>
              SwiGLU splits the expansion into <b>two branches</b>, using one as a <b>gate</b> to modulate the other:
            </p>
            <p style={{ fontFamily: 'var(--mono)', fontSize: 13, color: 'var(--accent-2)' }}>
              SwiGLU(x) = ( Swish(x·W_g) ⊙ (x·W_u) ) · W_d
            </p>
            <ul>
              <li><b>value branch</b> <code>x·W_u</code>: candidate information.</li>
              <li><b>gate branch</b> <code>Swish(x·W_g)</code>: a <b>soft switch</b> for each hidden unit (Swish curve on the right),
                deciding <b>how much of the value branch to let through</b>.</li>
              <li><b>⊙ elementwise product</b> → expanded hidden, then <code>·W_d</code> back to d, added to the residual stream as Δ.</li>
            </ul>
            <div className="note">
              Why 3 matrices is still better: gating is more expressive than plain ReLU, and SwiGLU is reliably better at equal compute.
              Because of the extra branch, d_ff is usually about <b>(2/3)·4d</b> so the parameter count matches a standard FFN.
            </div>
            <div className="note" style={{ marginTop: 8 }}>
              Looking ahead to Ch12: <b>MoE</b> just replicates this FFN into many "experts", routing each token to only a few —
              the FFN is the <b>single-expert prototype</b> of MoE.
            </div>
          </>
        ) : (
          <>
      ...
      <>
        <h3>{t('SwiGLU:一个 token 的前向(门 ⊙ 值 → 降维)', "SwiGLU: one token's forward pass (gate ⊙ value → project back)")}</h3>
        <FigureBoard renderSvg={renderSwiGLU} baseCell={28} fullCell={42}
          controls={controls} onPageStep={onPageStep} />

        <h3 style={{ marginTop: 14 }}>{t('激活:Swish vs ReLU', 'Activation: Swish vs ReLU')}</h3>
        <div style={{ overflowX: 'auto', paddingBottom: 8, background: 'var(--bg)',
          border: '1px solid var(--border)', borderRadius: 10, padding: '8px 10px' }}>
          {renderActivation()}
        </div>

        <h3 style={{ marginTop: 16 }}>{t('关键对比:FFN 逐 token,注意力跨 token', 'Key contrast: FFN per-token, attention across tokens')}</h3>
        <div style={{ overflowX: 'auto', paddingBottom: 8, background: 'var(--bg)',
          border: '1px solid var(--border)', borderRadius: 10, padding: '8px 10px' }}>
          {renderContrast()}
        </div>
