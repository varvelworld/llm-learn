...
export default function Ch18Engram({ prev, next }) {
  ...
  const renderLookup = (cell) => {
    ...
    els.push(<text key="seqL" x={8} y={top - 4} fontFamily={T.font} fontSize={9} fill={T.c.dim}>输入序列(蓝=最近 {NG} 个 token = n-gram)</text>)
    // 哈希说明行
    const y2 = top + cs + 28
    ...
    els.push(<text key="hash" x={8} y={y2} fontFamily={T.font} fontSize={11} fill={T.c.text}>
      hash(<tspan fill={T.c.accent}>{ngTxt}</tspan>) <tspan fill={T.c.dim}>= FNV 整数哈希 % {B} →</tspan> 槽 <tspan fill={T.c.warn} fontWeight={700}>#{look.bucket}</tspan></text>)
    // 记忆库:B 列 × DM 行,命中列高亮
    const bankX = 8
    const bankY = y2 + 18
    ...
    els.push(<text key="bankL" x={bankX} y={bankY - 5} fontFamily={T.font} fontSize={9.5} fill={T.c.dim}>记忆库(存于 CPU DRAM,{B} 槽)</text>)
    els.push(<text key="hitL" x={bankX + B * bcs + 10} y={bankY + DM * bcs / 2} fontFamily={T.font} fontSize={10.5}
      fill={T.c.warn}>← 取出这条<tspan x={bankX + B * bcs + 10} dy={14} fill={T.c.accent2}>记忆嵌入 m,补进 h</tspan>
      <tspan x={bankX + B * bcs + 10} dy={16} fill={T.c.dim}>O(1):1 次哈希 + 取 1 列</tspan></text>)
    const W = Math.max(8 + SENT.length * tw + 6, bankX + B * bcs + 170)
  ...
  const renderU = (cell) => {
    ...
    els.push(<text key="bandL" x={X(0.225)} y={y0 - 3} textAnchor="middle" fontFamily={T.font} fontSize={9} fill={T.c.accent2}>最优 ~20–25%</text>)
    // 轴
    ...
    els.push(<text key="xl" x={x0 + W} y={y0 + Hh + 14} textAnchor="end" fontFamily={T.font} fontSize={9.5} fill={T.c.dim}>记忆占比 f →</text>)
    els.push(<text key="yl" x={x0 - 2} y={y0 - 3} textAnchor="end" fontFamily={T.font} fontSize={9.5} fill={T.c.dim}>损失↓</text>)
    els.push(<text key="x0" x={x0} y={y0 + Hh + 14} textAnchor="middle" fontFamily={T.font} fontSize={9} fill={T.c.dim}>0(全计算)</text>)
    els.push(<text key="x1" x={x0 + W} y={y0 + Hh + 26} textAnchor="end" fontFamily={T.font} fontSize={9} fill={T.c.dim}>1(全记忆)</text>)
    // 曲线
    const pts = []
  ...
  const lookupControls = (
    <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
      <span style={{ color: 'var(--text-dim)', width: 120 }}>n-gram 位置</span>
      <input type="range" min={NG - 1} max={SENT.length - 1} step={1} value={p} onChange={(e) => setP(+e.target.value)} style={{ width: 150 }} />
      <b style={{ fontFamily: 'var(--mono)', color: 'var(--accent)' }}>{SENT.slice(look.lo, p + 1).map((t) => t[0]).join(' ')}</b>
    </label>
  )
  const uControls = (
    <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
      <span style={{ color: 'var(--text-dim)', width: 120 }}>记忆占比 f</span>
...
\\qquad(\\textbf{O(1)})`
  ...
  return (
    <ChapterLayout kicker="第二部分 · DeepSeek · Ch18" title="Engram:把「记忆」从「计算」里拆出来" prev={prev} next={next}>
      <>
        <p>
      ...
      <>
        ...
        <h3 style={{ marginTop: 18 }}>图② 稀疏分配律:记忆 vs 计算怎么分</h3>
        <p style={{ fontSize: 13, color: 'var(--text-dim)', margin: '4px 0 10px' }}>
        ...
        <div style={{ marginTop: 10, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px' }}>
          <div style={{ fontSize: 12.5, color: 'var(--text-dim)' }}>
            当前记忆占比 <b style={{ color: 'var(--accent)' }}>{fPct}%</b>:验证损失
            <b style={{ color: Math.abs(f - ALLOC_OPTIMUM) < 0.05 ? 'var(--accent2)' : 'var(--warn)' }}> {curLoss.toFixed(3)}</b>
            {Math.abs(f - ALLOC_OPTIMUM) < 0.05 ? '(就在最优带里 ✓)' : `(最优在 ~22%,损失 ${allocLoss(ALLOC_OPTIMUM).toFixed(3)})`}。
          </div>
        </div>
      </>
    </ChapterLayout>
