...
export default function Ch17MHC({ prev, next }) {
  ...
  const render = (cell) => {
    ...
    // —— 左:流间混合矩阵 H^res —— //
    els.push(<text key="mt" x={mx} y={16} fontFamily={T.font} fontSize={11} fill={T.c.dim}>流间混合矩阵 Hʳᵉˢ (n={N})</text>)
    for (let i = 0; i < N; i++) {
      for (let j = 0; j < N; j++) {
        const v = d.M[i][j]
    ...
    els.push(<text key="rl" x={mx + N * cs + sumGap} y={my - 6} fontFamily={T.font} fontSize={8.5} fill={T.c.dim}>行和</text>)
    els.push(<text key="cl" x={mx + N * cs + sumGap} y={my + N * cs + 13} fontFamily={T.font} fontSize={8.5} fill={T.c.dim}>← 列和</text>)
    ...
    els.push(<text key="cht" x={chX} y={16} fontFamily={T.font} fontSize={11} fill={T.c.dim}>信号增益 ‖Hˡ x‖ / ‖x‖(跨 {L} 层,log 轴)</text>)
    els.push(<text key="chx" x={chX + chW} y={chTop + chH + 13} textAnchor="end" fontFamily={T.font} fontSize={8.5} fill={T.c.dim}>层 → {L}</text>)
  ...
  const controls = (
    <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
      <span style={{ color: 'var(--text-dim)', width: 130 }}>Sinkhorn 迭代 k</span>
      ...
      <span style={{ color: 'var(--text-dim)', fontSize: 12 }}>{k === 0 ? '(未约束 HC)' : d.isDS ? '(已双随机 = mHC)' : '(归一中…)'}</span>
    </label>
  )

  const hcTex = `\\underbrace{x_{l+1}=x_l+f(x_l)}_{\\textcolor{#6ea8fe}{\\text{单流残差}}}
\\;\\longrightarrow\\;
\\underbrace{H_{l+1}=B^{\\top}f(A_m^{\\top}H_l)+\\textcolor{#f0a35e}{H^{\\text{res}}}\\,H_l}_{\\textcolor{#7ee787}{\\text{HC:}\\,n\\text{ 条流}+\\text{流间混合}}}`

  const dsTex = `\\textcolor{#f0a35e}{H^{\\text{res}}}\\in\\text{Birkhoff(双随机)}:\\quad
H^{\\text{res}}_{ij}\\ge0,\\;\\;\\sum_{j}H^{\\text{res}}_{ij}=1,\\;\\;\\sum_{i}H^{\\text{res}}_{ij}=1
\\;\\;\\Rightarrow\\;\\;\\rho(H^{\\text{res}})=1`

  return (
    <ChapterLayout kicker="第二部分 · DeepSeek-V4 · Ch17" title="mHC:把残差从「一条流」变成「稳的多条流」" prev={prev} next={next}>
      <>
        <p>
      ...
      <>
        <h3>Sinkhorn 把混合矩阵推成双随机 → 信号增益从爆炸到稳定</h3>
