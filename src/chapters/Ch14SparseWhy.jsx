...
export default function Ch14SparseWhy({ prev, next }) {
  ...
  const renderTrace = (cell) => {
    ...
    add(blk(x, qv, { vmax: ioMax, title: `q「${tokens[r]}」` }), 0, '·', 20)
    add(blk(x, Kt, { vmax: ioMax, title: `Kᵀ(前 ${m} 个键)`, colLabels: tokens.slice(0, m) }), 0, '=', 24)
    add(blk(x, sc, { vmax: scMax, colKept: keptCol, title: '分数 q·kⱼ/√d' }), 0, 'softmax→', 66)
    add(blk(x, wt, { vmax: 1, dec: 2, colKept: keptCol, title: '权重(=上方该行)' }), 0, '·', 20)
    add(blk(x, Vr, { vmax: ioMax, rowKept: keptCol, title: `V(前 ${m} 个值)`, rowLabels: tokens.slice(0, m) }), 0, '=', 24)
  ...
  const renderSvg = (cell) => {
    ...
    const W = gx + n * cell + 16
    const H = gy + n * cell + 34
    const show = n <= 9
    return (
      <svg width={W} height={H} style={{ display: 'block', minWidth: W }}>
        {/* 轴说明 */}
        <text x={gx} y={12} fontFamily={T.font} fontSize={11} fill={T.c.dim}>键(被注目的 token)→</text>
        <text x={4} y={gy - 4} fontFamily={T.font} fontSize={11} fill={T.c.dim}>查询↓</text>
  ...
  return (
    ...
    >
      ...
      <>
        ...
        <h3 style={{ marginTop: 18 }}>要算多少个注意力分数(toy · 当前 n={n})</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            { name: '满注意力 O(n²)', val: fullPairs, color: 'var(--hot)' },
            { name: `稀疏 Top-${kk}`, val: sparse ? sparsePairs : fullPairs, color: 'var(--accent-2)' },
          ].map((b) => (
        ...
        <div className="cost-panel" style={{ marginTop: 12 }}>
          <div className="cost-row">
            <span>toy:满 vs 稀疏(Top-{kk})的分数个数</span>
            <b>{fullPairs} → {sparse ? sparsePairs : fullPairs}</b>
          </div>
          <div className="cost-divider" />
          <div className="cost-row" style={{ fontSize: 12 }}>
            <span>真实参照:128K 上下文,DSA 选 top-{fmt(TOPK_REAL)}</span>
            <b style={{ color: 'var(--warn)' }}>≈ {speedup}× 更少的分数计算</b>
          </div>
          <div className="cost-row" style={{ fontSize: 12 }}>
            <span>复杂度</span>
            <b>O(n²) → O(n·k),k 固定 ⇒ 近似线性</b>
          </div>
        </div>
      </>
    </ChapterLayout>
