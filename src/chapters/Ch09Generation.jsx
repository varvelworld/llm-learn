...
export default function Ch09Generation({ prev, next }) {
  ...
  const [t, setT] = useState(3)         // 当前已生成到第 t 个 token
  ...
  const setStep = useCallback((v) => { setPlaying(false); setT(Math.max(1, Math.min(L, v))) }, [L])
  const onPageStep = useCallback((d) => setT((v) => Math.max(1, Math.min(L, v + d))), [L])
  ...
  const onLen = useCallback((v) => { setPlaying(false); const n = Math.round(v); setL(n); setT(n) }, [])
  // 自动播放:每 800ms 走一步,到末尾自停
  useEffect(() => {
    if (!playing) return
    if (t >= L) { setPlaying(false); return }
    const id = setTimeout(() => setT((v) => Math.min(L, v + 1)), 800)
    return () => clearTimeout(id)
  }, [playing, t, L])
  const togglePlay = useCallback(() => {
    if (playing) { setPlaying(false); return }
    if (t >= L) setT(1) // 已到末尾 → 从头重播
    setPlaying(true)
  }, [playing, t, L])

  // 整段所有位置的 q/k/v(确定性);decode 第 t 步只用到前 t 个
  const Qa = useMemo(() => seededMatrix(CAP, D, 11), [])
  const Ka = useMemo(() => seededMatrix(CAP, D, 22), [])
  const Va = useMemo(() => seededMatrix(CAP, D, 33), [])

  const step = useMemo(() => {
    const Kc = Ka.slice(0, t)
    const Vc = Va.slice(0, t)
    const q = Qa[t - 1]
  ...
  const tokens = useMemo(() => Array.from({ length: t }, (_, i) => labelFor(i)), [t])
  // 缓存行的明暗:最后一行(本步新算)亮,其余(缓存复用)淡
  const fade = useMemo(() => Array.from({ length: t }, (_, i) => (i === t - 1 ? 1 : 0.4)), [t])
  // 当前聚焦的缓存位置(默认指向本步新行);点积拆解用
  const sel = selRaw < 0 ? t - 1 : Math.min(selRaw, t - 1)
  ...
  const renderSvg = (cell0) => {
    // 序列长时自动缩小格子,长缓存也能整屏看
    const cell = t > 22 ? Math.round(cell0 * 0.42) : t > 12 ? Math.round(cell0 * 0.6) : cell0
    const lw = T.labelW
    const rowLabels = t <= 12 ? tokens : undefined
    const qCol = step.q.map((v) => [v]) // q_t 作为 D×1 列,躺在分数列上方
    // ① 交叉网格:K缓存(t×D) · q_t(D×1) = 分数(t×1)
    const L = matmulLayout({ m: t, k: D, p: 1, cell, labelW: lw, colLabelH: T.colLabelH, gap: T.gap })
      ...
      <svg width={svgW} height={svgH} style={{ display: 'block', minWidth: svgW }}>
        ...
        <g transform={`translate(0,${topY})`}>
          <MatMul x={0} y={0} A={step.Kc} Bt={qCol} result={step.scoreCol} cell={cell}
            ...
            aLabel="K缓存" bLabel="q_t" resultLabel="分数" showValues={show} />
        </g>
        <text x={L.A.x} y={baseY + mh + 36} fontFamily={T.font} fontSize={11} fill={T.c.accent}>
          ↑ 蓝框 = 第 {sel + 1} 个缓存键 · 末行(亮)=本步新算 k_t,其余(淡)=缓存复用
        </text>

        {/* softmax → 权重列 */}
        <text x={xW} y={baseY - 6} fontFamily={T.font} fontSize={11} fill={T.c.accent2}>权重</text>
        ...
        <text x={xO} y={baseY - 6} fontFamily={T.font} fontSize={11} fill={T.c.accent2}>输出 → 下一个词</text>
        ...
        <Edge from={{ x: vRight, y: baseY + cell / 2 }} to={{ x: xO, y: baseY + cell / 2 }} label={'加权和'} />
      </svg>
    )
  }
  ...
  const controls = (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', fontSize: 13 }}>
      <button className="btn" onClick={() => setStep(t - 1)} disabled={t <= 1}>◀ 上一步</button>
      <button className="btn" onClick={togglePlay} style={playing ? { borderColor: 'var(--accent)', color: 'var(--accent)' } : undefined}>
        {playing ? '⏸ 暂停' : (t >= L ? '↻ 重播' : '▶ 自动播放')}
      </button>
      <button className="btn" onClick={() => setStep(t + 1)} disabled={t >= L}>下一步 ▶</button>
      <span style={{ color: 'var(--text-dim)' }}>第 <b style={{ color: 'var(--accent)' }}>{t}</b> / {L} 步</span>
  ...
  return (
    <ChapterLayout kicker="第 9 章 · 生成" title="自回归生成 + KV 缓存" prev={prev} next={next}>
      <>
        <p>
      ...
      <>
        ...
        <div className="matmul-expand">
          <div style={{ marginBottom: 4 }}>
            分数[<b style={{ color: 'var(--accent)' }}>{tokens[sel]}</b>] = 第 {sel + 1} 个缓存键 k · q_t 逐位相乘再相加:
          </div>
        ...
        <h3 style={{ marginTop: 18 }}>缓存开销(到第 {t} 步)</h3>
        <div className="cost-panel">
          <div className="cost-row"><span>KV 缓存(toy:数字个数 = 2·t·d)</span><b>{kvNums}</b></div>
          <div className="cost-row"><span>每步计算量 · 无缓存(重算前面所有)</span><b style={{ color: 'var(--warn)' }}>O(t²)</b></div>
          <div className="cost-row"><span>每步计算量 · 有缓存(只算新行)</span><b style={{ color: 'var(--accent-2)' }}>O(t)</b></div>
          <div className="cost-divider" />
          <div className="cost-row" style={{ fontSize: 12 }}>
            <span>缓存随序列</span><b>线性增长 → 长上下文吃显存 + 解码带宽瓶颈</b>
          </div>
        </div>
      </>
    </ChapterLayout>
